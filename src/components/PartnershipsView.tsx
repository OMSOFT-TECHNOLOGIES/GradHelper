...
export function PartnershipsView({ user }: PartnershipsViewProps) {
  ...
  const handleApplyPartnership = (applicationData: any) => {
    setPartnershipData(prev => ({
      ...
      referralCode: `GRAD-${user.name.toUpperCase().replace(' ', '-')}-2025`
    }));

    addNotification({
      type: 'system',
      title: 'Partnership Application Submitted',
      message: 'Your application to become a school representative has been submitted for review.',
      userId: user.id,
      userRole: 'student',
      ...
      {partnershipData.status === 'pending' && (
        <div className="status-banner pending">
          ...
          <div>
            ...
            <p>Your partnership application is being reviewed. You'll be notified once approved.</p>
          </div>
        </div>
      )}
